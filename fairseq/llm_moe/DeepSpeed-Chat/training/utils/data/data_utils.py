...
def shard_data(all_data, num_shards, shard_id):
    """
    将数据分片成num_shards个子列表，返回第i个子列表。
    """
    assert shard_id >= 0 and shard_id < num_shards, "i必须大于等于0并且小于num_shards！"
    num_data = len(all_data)
    assert num_data >= num_shards, "数据量必须大于等于num_shards！"

    shard_size = num_data // num_shards
    remainder = num_data % num_shards

    if shard_id < remainder:
        start = shard_id * (shard_size + 1)
        end = start + shard_size + 1
    else:
        start = shard_id * shard_size + remainder
...
def batchify(lst, batch_size):
    """
    将一个list按照batch_size大小分组

    :param lst: 要分组的list
    :type lst: list
    :param batch_size: 每个batch的大小
    :type batch_size: int
    :return: 分组后的list，其中每个元素是一个batch
    :rtype: list
    """
