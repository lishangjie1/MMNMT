...
def main():
    ...
    # engine.module.eval()
    # model = engine.module
    model = model.to(device=device, dtype=torch.bfloat16 if args.dtype == 'bf16' else torch.float16).eval()
    ...
    for step, raw_batch in enumerate(batches):
        ...
        for raw_sample, raw_output in zip(raw_batch, raw_outputs):
            ...
            output_samples.append(pure_output)

        n_samples = len(input_ids)

        output_samples_split = []

        for idx in range(n_samples):
            input_id_ = [input_ids[idx], ]

            batch = {
                "input_ids": pad_sequence(input_id_, padding_value=tokenizer.pad_token_id)
            }
            ...
            # print(batch['input_ids'][idx:idx+1])

            # outputs = model.generate(batch['input_ids'][idx:idx+1],
            #                          attention_mask=batch['attention_mask'][idx:idx+1],
            #                          **generate_kwargs)

            raw_outputs = tokenizer.batch_decode(outputs, skip_special_tokens=True)

            pure_output = raw_outputs[0].replace(raw_batch[idx]['input'], '').strip()
            output_samples_split.append(pure_output)

        print(output_samples)
        print(output_samples_split)
        exit(1)
