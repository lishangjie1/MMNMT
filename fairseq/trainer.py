        ...
        if bexists or bexists_share:
            ...
            try:
                if not bexists and bexists_share:
                    ...
                    model_state_dict = self.model.state_dict()
                    initialized_key = []
                    random_initialized_key = []
                    for key in state["model"]:
                        if key.startswith("decoder") and "embed_tokens" not in key and "output_projection" not in key:
                            continue
                        if key in model_state_dict:
                            logger.info(f'initialize {key} from the loaded model')
                            initialized_key.append(key)
                            model_state_dict[key] = state['model'][key]
                        else:
                            # init expert parameters
                            # add noise and initialize moe experts
                            def random_mask(params, prob=0.1):
                                mask = torch.rand(params.shape) < prob
                            ...
                            while True:
                                if "fc1" in key:
                                    new_key = re.sub("fc1", f"moe_module.experts.{cnt}.fc1", key)
                                else:
                                    new_key = re.sub("fc2", f"moe_module.experts.{cnt}.fc2", key)
                                if new_key not in model_state_dict:
                                    break
                                logger.info(f'initialize {new_key} with randomly masked (prob={prob}) {key} from the loaded model')
                                model_state_dict[new_key] = random_mask(state['model'][key], prob=prob)
                                initialized_key.append(new_key)
                                cnt += 1
                    #loading moe multi-gate trained from scratch
                    # merge_filename = f"/data/oss_bucket_0/moe_models/moe_model_base_multi-valid_poly/checkpoint_4_100000-rank-{torch.distributed.get_rank()}.pt"
                    # logger.info(f'loading decoder parameters from model: {merge_filename}')
                    
                    # merge_state = checkpoint_utils.load_checkpoint_to_cpu(
                    #     merge_filename,
                    #     load_on_all_ranks=True,
                    #     is_moe=True,
                    # )
                    # for key in merge_state["model"]:
                    #     if "embed_tokens" in key or "output_projection" in key:
                    #         continue
                    #     if key.startswith("decoder.") and key in model_state_dict:
                    #         logger.info(f'initialize {key} from the loaded merge model')
                    #         initialized_key.append(key)
                    #         model_state_dict[key] = merge_state['model'][key].to(model_state_dict[key])




                    initialized_key = set(initialized_key)
                    for key in model_state_dict:
                        if key not in initialized_key:
                            random_initialized_key.append(key)
                    print("random initialized key: ", random_initialized_key)
