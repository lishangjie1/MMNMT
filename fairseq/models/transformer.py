...
from fairseq.data.multilingual.multilingual_utils import get_langs_from_lang_ids
...
def fsdp_wrap_expert(args, layer, min_num_params=0):
    ...
    if hasattr(layer.moe_module, "moe_layer"):
        moe_layer = layer.moe_module.moe_layer
    else:
        moe_layer = layer.moe_module

    process_group = moe_layer.expert_group
...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--freeze-share-parameters', default=False, action='store_true',
                            help="if true freeze share parameters")
        parser.add_argument('--freeze-encoder-share-parameters', default=False, action='store_true',
                            help="if true freeze share parameters")
        parser.add_argument('--freeze-decoder-share-parameters', default=False, action='store_true',
                            help="if true freeze share parameters")
        # fmt: on
    ...
    def build_model(cls, args, task):
        ...
        if getattr(args, "freeze_share_parameters", False) or getattr(args, "freeze_encoder_share_parameters", False):
            # freeze share part except gate
            for p in encoder.parameters():
                if hasattr(p, "expert") and p.expert == True:
                    continue
                p.requires_grad = False
            
            for i in range(len(encoder.layers)):
                if encoder.layers[i].is_moe_layer:
                    if args.use_moe_cmr:
                        for p in encoder.layers[i].moe_module.moe_layer.gate.parameters():
                            p.requires_grad = True
                    else:
                        for p in encoder.layers[i].moe_module.gate.parameters():
                            p.requires_grad = True


        if getattr(args, "freeze_share_parameters", False) or getattr(args, "freeze_decoder_share_parameters", False):
            ...
            for i in range(len(decoder.layers)):
                if decoder.layers[i].is_moe_layer:
                    if args.use_moe_cmr:
                        for p in decoder.layers[i].moe_module.moe_layer.gate.parameters():
                            p.requires_grad = True
                    else:
                        for p in decoder.layers[i].moe_module.gate.parameters():
                            p.requires_grad = True
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.lang2id = dict([(lang, idx) for idx, lang in enumerate(args.langs)])
    ...
    def forward_scriptable(
        ...
        if src_lang_id is not None:
            src_lang_embeddings = self.forward_embedding(src_lang_id.unsqueeze(1))[1] 
            batch_langs = get_langs_from_lang_ids(src_lang_id, self.dictionary, self.args.lang_tok_style) 
            batch_lang_ids = torch.IntTensor([self.lang2id[lang] for lang in batch_langs]).to(src_lang_embeddings.device)
        else:
            src_lang_embeddings, batch_langs, batch_lang_ids = None, None, None
        # record lang id
        ...
        for layer in self.layers:
            x, l_aux_i = layer(
                x, encoder_padding_mask=encoder_padding_mask if has_pads else None, lang_embeddings=src_lang_embeddings, batch_lang_ids=batch_lang_ids
            )
            if return_all_hiddens:
    ...
    def collect_expert_choices(self, src_tokens):
        ...
        for layer in self.layers:
            if layer.is_moe_layer:
                expert_choices=layer.moe_module.metadata['expert_choices'] # (e,s)
    ...
    def collect_cmr_choices(self, src_tokens):
        ...
        for layer in self.layers:
            if layer.is_moe_layer:
                cmr_choices=layer.moe_module.metadata['cmr_choices'] # (s,)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def collect_expert_choices(self, tgt_tokens):
        ...
        # for inference: tgt_tokens[:,-1].view(-1), get expert choices for the last previous token
        # for validation: tgt_tokens.view(-1)
        ...
        for layer in self.layers:
            if layer.is_moe_layer:
                expert_choices=layer.moe_module.metadata['expert_choices']
                #print(tgt_tokens.shape, expert_choices.shape)
    ...
    def collect_cmr_choices(self, tgt_tokens):
        ...
        for layer in self.layers:
            if layer.is_moe_layer:
                cmr_choices=layer.moe_module.metadata['cmr_choices']
    ...
    def extract_features_scriptable(
        ...
        if tgt_lang_id is not None:
            tgt_lang_embeddings = self.forward_embedding(tgt_lang_id.unsqueeze(1))[1] 
            batch_langs = get_langs_from_lang_ids(tgt_lang_id, self.dictionary, self.args.lang_tok_style) 
            batch_lang_ids = torch.IntTensor([self.lang2id[lang] for lang in batch_langs]).to(tgt_lang_embeddings.device)
        else:
            tgt_lang_embeddings, batch_langs, batch_lang_ids = None, None, None
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, _, l_aux_i, l_ctr_i = layer(
                x,
                ...
                batch_lang_ids=batch_lang_ids
            )
            l_aux.append(l_aux_i)
            l_ctr.append(l_ctr_i)
        ...
        return x, {"attn": [attn], "inner_states": inner_states, "l_aux": l_aux, "l_ctr": l_ctr}
