...
class CMRLayer(torch.nn.Module):
    ...
    def forward(
        ...
        **kwargs: Any
        ...
        #x_out = x_ffn + x_moe * moe_gates.unsqueeze(-1) 

        x_out = 0.5 * x_ffn + 0.5 * x_moe # residule connection
        ...
        #l_aux["cmr_gate_loss_num"] = 0
        #l_aux["cmr_gate_loss_denom"] = 0

        #self.moe_layer.metadata["moe_rate"] = (gates).mean().data
    ...
    def record_cmr_choices(self, gates):
        self.moe_layer.metadata['cmr_choices'] = gates.clone().detach() # (s,)