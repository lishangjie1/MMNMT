...
class MOELayer(Base):
    ...
    def forward(self, *input: Tensor, input_padding_mask=None, **kwargs: Any) -> Tensor:
        ...
        if expected_bsz == 0:
            ...
            assert expected_dim >= reshaped_input_shape[0]
        ...
        batch_lang_ids = kwargs.get("batch_lang_ids", None)
        ...
        if self.gate.use_moe_lang_perception:
            ...
            assert batch_lang_ids.shape[0] == input.shape[0]
            ...
            padded_batch_lang_ids = torch.zeros(
                                (expected_dim, ),
                                dtype=batch_lang_ids.dtype, layout=batch_lang_ids.layout, device=batch_lang_ids.device)
            padded_batch_lang_ids[:batch_lang_ids.shape[0]] = batch_lang_ids
            batch_lang_ids = padded_batch_lang_ids
        ...
        if self.use_tutel:
            ...
            l_aux, combine_weights, dispatch_mask, self.metadata = self.gate(reshaped_input, reshaped_input_padding_mask, has_tutel=False, lang_embeddings=lang_embeddings, batch_lang_ids=batch_lang_ids)
