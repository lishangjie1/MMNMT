    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, is_moe_layer=False):
        ...
        if not self.is_moe_layer or getattr(args, "alternate_ffn_embed_dim", 0.0) > 0:
            ...
            moe_layer = self.build_moe_layer(gate, experts, args)
            self.is_use_moe_cmr = args.use_moe_cmr

            if args.use_moe_cmr:
                from fairseq.modules.moe.moe_cmr_layer import CMRLayer
                share_expert = self.build_share_ffn(
                        args, self.embed_dim, ffn_dim, self.dropout_module
                    )
                self.moe_module = CMRLayer(moe_layer, share_expert, self.embed_dim, getattr(args, "moe_cmr_dropout", 0.0), use_cmr_lang_perception=getattr(args, "use_cmr_lang_perception", False))
    ...
    def forward(self, 
        ...
        if not self.is_moe_layer or getattr(self.args, "alternate_ffn_embed_dim", 0.0) > 0:
            ...
            if batch_lang_ids is not None:
                batch_lang_ids = batch_lang_ids.unsqueeze(-1).expand((x_shape[0], x_shape[1]))
            # drop pad token
            if encoder_padding_mask is not None:
                ...
                if batch_lang_ids is not None:
                    batch_lang_ids = batch_lang_ids[nonpadding]
            else:
                ...
                if batch_lang_ids is not None:
                    batch_lang_ids = batch_lang_ids.reshape(-1)
 
            x, l_aux = self.moe_module(x, lang_embeddings=lang_embeddings, batch_lang_ids=batch_lang_ids)
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(
        ...
        if not self.is_moe_layer or getattr(self.args, "alternate_decoder_ffn_embed_dim", 0.0) > 0:
            ...
            def get_contrastive_loss(token_rep, sentence_rep, mask, temperature=0.2):
                
                
                max_batch_size = 20
                batch_size = token_rep.shape[0]
                seq_len = token_rep.shape[1]
                feature_dim = token_rep.shape[2]

                partial_mask = mask
                if max_batch_size < batch_size:
                    # subsampling

                    choice = np.random.choice(range(batch_size), max_batch_size, replace=False)
                    choice_mask = torch.zeros(batch_size, device=token_rep.device).bool()
                    choice_mask[choice] = True


                    token_rep = token_rep[choice_mask] # (new_bsz, seq_len, hidden)
                    sentence_rep = sentence_rep[choice_mask]
                    batch_size = max_batch_size

                    partial_mask = mask[choice_mask]
                ...
                anchor_feature = token_rep.transpose(0,1).unsqueeze(1).expand(seq_len, batch_size, batch_size, feature_dim) # T x B x B x C
                contrast_feature = sentence_rep.transpose(0,1).unsqueeze(2).expand(seq_len, batch_size, batch_size, feature_dim) # # T x B x B x C
                similarity_function = nn.CosineSimilarity(dim=-1)

                anchor_dot_contrast = similarity_function(anchor_feature, contrast_feature) # T x B x B
                print(anchor_dot_contrast[1][:,0])
                log_s = -torch.nn.LogSoftmax(1)(torch.div(anchor_dot_contrast, temperature))

                loss = torch.stack([log_s[i].diag() for i in range(seq_len)]).transpose(0, 1) # B x T
                

                loss = loss[partial_mask].sum() * mask.sum() / partial_mask.sum()

                print(f"ctr loss: {loss / mask.sum()}")
            ...
            #         nonpadding = torch.ones((b, t), device=x.device).bool()
            #     else:
            #         nonpadding = ~self_attn_padding_mask.bool() 
            #     #sentence_embeddings = x.mean(dim=1, keepdim=True).expand(x.shape) # B x T x C
            #     sentence_embeddings = (x * nonpadding.unsqueeze(-1)).sum(dim=1) / (nonpadding.float()).sum(dim=1).unsqueeze(-1) # B x C
            #     sentence_embeddings = sentence_embeddings.unsqueeze(1).expand(x.shape) # B x T x C


            #     ctr_loss = get_contrastive_loss(x, sentence_embeddings, nonpadding)


                
            #     print(f"inner sim: {torch.nn.CosineSimilarity()(x[0][1], x[0][:min(10, t)])}, outer_sim: {torch.nn.CosineSimilarity()(x[0][1], x[:min(10, b),1])}")
            # else:
            ctr_loss = None
        ...
        return x, attn, None, l_aux, ctr_loss
