...
def get_langs_from_lang_ids(lang_ids, dict, style):
    import re
    TOKEN_STYLES: Dict[str, str] = {
        LangTokStyle.mbart.value: "[\w+]",
        LangTokStyle.multilingual.value: "__(\w+)__",
    }

    assert style in TOKEN_STYLES
    if lang_ids[0] == 0:
        # fake sample
        return ['en'] * len(lang_ids)
    format_langs = [dict.symbols[lang_id] for lang_id in lang_ids]
    pattern = re.compile(TOKEN_STYLES[style])
    langs = [re.search(pattern, lang).group(1) for lang in format_langs]
    return langs
