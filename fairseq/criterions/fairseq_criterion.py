...
class MoECriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        3) logging outputs to display while training
        """
        loss, inner_loss, moe_loss, cmr_loss, ctr_loss, moe_metadata, sample_size, logging_output = self.compute_loss(model, sample, reduce=reduce)
        ...
        logging_output["ctr_loss"] = ctr_loss.data if torch.is_tensor(ctr_loss) else ctr_loss
    ...
    def compute_loss(self, model, sample, reduce=True):
        ...
        for l_aux in net_output[1]["l_aux"]:
            if l_aux is not None:
                ...
                gate_count += 1
        for l_ctr in net_output[1]["l_ctr"]:
            if l_ctr is not None:
                ctr_loss += l_ctr
                ctr_count += 1
        if ctr_count > 0:
            ctr_loss = ctr_loss / ctr_count
        ...
        loss = inner_loss + self.gate_loss_weight * gate_loss + self.cmr_gate_loss_weight * cmr_gate_loss + ctr_loss
        
        return loss, inner_loss, gate_loss, cmr_gate_loss, ctr_loss, self.get_moe_metadata(model), sample_size, logging_output
    ...
    def reduce_moe_metrics(logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "ctr_loss", ctr_loss_sum / sample_size, sample_size, round=8
        )
