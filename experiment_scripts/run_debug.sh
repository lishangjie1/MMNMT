...
SAVE="/mnt/nas/users/lsj/moe/models/moe_model_reload"
...
python train.py $DATA \
  ...
  --log-interval 10 \
  --save-interval-updates 100 --save-dir $SAVE --keep-interval-updates 2 --no-epoch-checkpoints \
  --dataset-impl "mmap" \
  ...
  --use-moe-cmr --moe-cmr-dropout 0.2 --cmr-gate-loss-p 0 --cmr-gate-loss-wt 0 \
  --restore-file "/mnt/nas/users/lsj/moe/models/dense_model_test/checkpoint_last.pt" \
  --reset-optimizer --reset-dataloader --reset-meters --reset-lr-scheduler \
  --freeze-encoder-share-parameters \
  --weight-decay 0.1
